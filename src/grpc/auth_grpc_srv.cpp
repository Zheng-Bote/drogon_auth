...
static bool is_authorized(::grpc::ServerContext* context) {
    std::string expected_key = drogon_auth::utils::ConfigUtil::get_string("GRPC_API_KEY", "");
    if (expected_key.empty()) return true; // Security bypass if not configured, or could be strict
...
#define CHECK_GRPC_AUTH() \
    if (!is_authorized(context)) { \
        return ::grpc::Status(::grpc::StatusCode::UNAUTHENTICATED, "Invalid or missing x-api-key"); \
    }

::grpc::Status AuthGrpcServiceImpl::GetUserStatus(::grpc::ServerContext* context,
    ...
    try {
        // 1. Check session
        auto res = db->execSqlSync("SELECT user_id FROM sessions WHERE session_token = $1 AND expires_at > CURRENT_TIMESTAMP", session_id);
        ...
        auto user_res = db->execSqlSync("SELECT is_active FROM users WHERE id = $1", user_id);
        if (!user_res.empty()) {
...
::grpc::Status AuthGrpcServiceImpl::CheckHealth(::grpc::ServerContext* context,
...
::grpc::Status AuthGrpcServiceImpl::GetUserProfile(::grpc::ServerContext* context,
    ...
    try {
        auto res = db->execSqlSync(
            "SELECT first_name, last_name, preferred_language, locale, timezone, attributes "
        ...
        response->set_preferred_language(res[0]["preferred_language"].as<std::string>());
...
::grpc::Status AuthGrpcServiceImpl::GetUserCommunications(::grpc::ServerContext* context,
                                                         const UserRequest* request,
    ...
    try {
        auto res = db->execSqlSync(
            "SELECT channel, address, is_active, verified "
